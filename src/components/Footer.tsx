      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 md:grid-cols-3 md:px-8">
        <div>
          <div className="flex items-center gap-3">
            <img src={logo} alt="Gastrô Dalí" className="h-14 w-14 object-contain" />
            <span className="font-display text-2xl">Gastrô Dalí</span>
          </div>
          <p className="mt-5 max-w-xs text-sm leading-relaxed text-muted-foreground">
            Gastronomia surreal no coração de Muriaé. Cada prato, uma obra de arte.
          </p>
          <a
          ...
          >
            <Instagram className="h-4 w-4" /> @gastrodali
          </a>
        </div>
        <div>
          <h4 className="mb-4 font-display text-lg text-primary">Contato</h4>
          <ul className="space-y-3 text-sm text-foreground/85">
            <li className="flex items-start gap-2">
              <MapPin className="mt-0.5 h-4 w-4 text-secondary" />
              <span>Boulevard Center Shopping<br />Av. Dr. Passos, 180, Centro<br />Muriaé/MG</span>
            </li>
        ...
        <div>
          <h4 className="mb-4 font-display text-lg text-primary">Navegue</h4>
